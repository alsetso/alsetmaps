...
-- Fixed to prevent double credit updates
...
BEGIN
    ...
    -- Log the transaction (trigger will update user_credits automatically)
...
AS $$
BEGIN
    -- Log the monthly credit additions (trigger will update user_credits automatically)
...
DROP TRIGGER IF EXISTS trigger_new_user ON public.accounts;
...
DROP TRIGGER IF EXISTS trigger_credit_balance_update ON public.credit_transactions;
