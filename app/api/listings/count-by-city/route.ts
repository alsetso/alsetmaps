...
export async function GET(request: NextRequest) {
  try {
    ...
    const { data: listingsCount, error } = await supabase
      .rpc('get_listings_count_by_city');
    ...
    if (error && error.code === 'PGRST202') {
      ...
      if (fallbackError) {
        console.error('Database error fetching listings count by city:', fallbackError);
        return NextResponse.json(
          { 
            success: false, 
            error: 'Database query failed',
            message: 'Unable to retrieve listing counts at this time',
      ...
      if (fallbackData && Array.isArray(fallbackData)) {
        fallbackData.forEach((listing: { city: string }) => {
      ...
      return NextResponse.json({
        ...
        meta: {
          ...
          totalListings: Object.values(countByCity).reduce((sum, count) => sum + count, 0),
    ...
    if (error) {
      console.error('Database error fetching listings count by city:', error);
      return NextResponse.json(
        { 
          success: false, 
          error: 'Database query failed',
    ...
    // Process RPC result
    const countByCity: Record<string, number> = {};
    if (listingsCount && Array.isArray(listingsCount)) {
      listingsCount.forEach((result: ListingCountResult) => {
    ...
    console.error('Unexpected error in GET /api/listings/count-by-city:', error);
