...
import { MagnifyingGlassIcon, PaperAirplaneIcon, InformationCircleIcon } from '@heroicons/react/24/outline';
import { useAuth } from '@/features/authentication/components/AuthProvider';
import { OptimizedCreditService } from '@/features/credit-system/services/optimized-credit-service';
import { OptimizedPropertySearchService } from '@/integrations/rapidapi/optimized-property-search-service';
...
interface OptimizedPropertySearchProps {
  ...
  onPropertyData?: (propertyData: any, fromCache: boolean) => void;
  className?: string;
}
...
export function OptimizedPropertySearch({ 
  ...
  const validateSearchCredits = async () => {
    ...
    try {
      const validation = await OptimizedCreditService.canPerformAction(
        user.id,
        'search_smart',
        1
      );
  ...
  const selectSuggestion = async (suggestion: AddressSuggestion) => {
    ...
    // If this is a smart search, perform the enhanced search
    if (searchMode === 'smart' && user?.id) {
      await performSmartSearch(suggestion);
    }
  };

  // Perform smart search with property data
  const performSmartSearch = async (suggestion: AddressSuggestion) => {
    if (!user?.id) {
      toast.error('Please log in to use smart search');
      return;
    }
    ...
    try {
      ...
      const searchResult = await OptimizedPropertySearchService.performSmartSearch(
        {
          address: suggestion.place_name,
      ...
      if (searchResult.success && searchResult.data) {
        ...
        if (searchResult.fromCache) {
          toast.success(`Property data loaded from cache (no credits consumed)`);
        } else {
          toast.success(`Smart search completed! ${searchResult.creditsConsumed} credit consumed`);
        }

        // Call the onPropertyData callback if provided
        if (onPropertyData) {
          onPropertyData(searchResult.data, searchResult.fromCache);
        }

        // Update credit status
        await validateSearchCredits();
      } else {
        toast.error(`Smart search failed: ${searchResult.error}`);
      }
    } catch (error) {
    ...
    <div className={`relative w-full max-w-2xl mx-auto px-4 md:px-0 ${className}`}>
      <div className="relative">
            ...
            {creditStatus ? (
              <div className="flex items-center justify-center space-x-2">
                <span className={`px-2 py-1 rounded-full ${
                  creditStatus.canProceed 
                    ? 'bg-green-100 text-green-700' 
                    : 'bg-red-100 text-red-700'
                }`}>
                  {creditStatus.message}
                  {creditStatus.availableCredits !== undefined && (
                    <span className="ml-1">({creditStatus.availableCredits} credits)</span>
                  )}
                </span>
                <InformationCircleIcon 
                  className="h-4 w-4 text-gray-400 cursor-help"
                  title="Smart search provides comprehensive property data, market analysis, and insights using 1 credit"
                />
              </div>
            ) : (
        ...
        {showSuggestions && suggestions.length > 0 && (
          <div 
            ref={suggestionsRef}
            className="absolute bottom-full left-0 right-0 mb-2 bg-white/95 backdrop-blur-md border border-white/30 rounded-xl shadow-2xl overflow-hidden z-50"
          >
